...
import { checkIfServicePending, checkTransactionReference, getLastServiceRecord, initiateServiceRecordTransaction, setTransactionStatusFailed, setTransactionStatusSuccess, submitBankingSlip, setRecordTransactionReference, setRecordTransactionReferenceWithOTP, } from './banking-cypher';
...
const bankingMutation = {
    BankServiceOffering: async (object, args, context) => {
        ...
        try {
            const paymentResponse = await axios(payOffering).catch((error) => throwToSentry('There was an error with the payment', error));
    ...
    SubmitBankingSlip: async (object, args, context) => {
        ...
        const checkIfAnyServicePending = rearrangeCypherObject(await session.run(checkIfServicePending, args).catch((error) => {
        ...
        if (checkIfAnyServicePending?.record?.properties?.transactionStatus) {
            throw new Error('You will have to confirm your initial self banking before uploading your banking slip');
        }
