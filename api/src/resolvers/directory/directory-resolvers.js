...
const directoryMutation = {
    ...
    CloseDownConstituency: async (object, args, context) => {
        ...
        const session = context.executionContext.session();
        try {
            const constituencyCheckResponse = await session.run(closeChurchCypher.checkConstituencyHasNoMembers, args);
            ...
            // Bacenta Leader must be removed since the Bacenta is being closed down
            await RemoveServant(context, args, permitAdmin('Council'), 'Constituency', 'Leader', true);
