...
import { aggregateBussingDataOnHigherChurches, aggregateVehicleBussingRecordData, checkArrivalTimeFromVehicle, checkArrivalTimes, checkBacentaMomoDetails, checkTransactionId, confirmVehicleByAdmin, getArrivalsPaymentDataCypher, getVehicleRecordWithDate, noVehicleTopUp, recordVehicleFromBacenta, removeVehicleRecordTransactionId, setSwellDate, setVehicleRecordTransactionId, setVehicleRecordTransactionSuccessful, setVehicleTopUp, uploadMobilisationPicture, } from './arrivals-cypher';
...
export const arrivalsMutation = {
    ...
    ConfirmVehicleByAdmin: async (object, args, context) => {
        ...
        await setBacentaStatus(bacentaId, context).catch((error) => console.log('Error Setting bacenta Status', error));
        const vehicleRecord = response.vehicleRecord.properties;
        const date = new Date().toISOString().slice(0, 10);
        const returnToCache = {
            id: vehicleRecord.id,
    ...
    SetVehicleSupport: async (object, args, context) => {
        ...
        const calculateTopUp = (vehicleCost) => {
            if (vehicleCost <= 50)
                return 0.5 * vehicleCost;
            if (vehicleCost <= 110)
                return 0.7 * vehicleCost;
        ...
        const calculateVehicleTopUp = (data) => {
            ...
            const amountToPay = data.vehicleCost * outbound - data.personalContribution;
        ...
        if (response.vehicleCost === 0 || vehicleTopUp <= 0) {
            const attendanceRes = await Promise.all([
                session.run(noVehicleTopUp, { ...args, vehicleTopUp }),
                sendBulkSMS([response.leaderPhoneNumber], joinMessageStrings([texts.arrivalsSMS.no_bussing_cost])),
            ]);
            vehicleRecord = rearrangeCypherObject(attendanceRes[0]);
            return vehicleRecord?.record.properties;
        }
    ...
    args, context) => {
        ...
        const recordResponse = rearrangeCypherObject(await session.run(checkTransactionId, args));
        ...
        const cypherResponse = rearrangeCypherObject(await session.run(setVehicleRecordTransactionId, args));
        ...
        try {
            ...
            if (res.data.code !== '000') {
                ...
                throwToSentry('There was an error processing payment', `${res.data.code} ${res.data.reason}`);
            }
            await session
            ...
            console.log('Money Sent Successfully to', vehicleRecord.momoNumber, res.data);
            return vehicleRecord;
        }
        catch (error) {
            throw new Error(`Money could not be sent! ${error.response.data.description}`);
        }
    },
